...
from mechanics.decorators import confirm_required, permission_required
from mechanics.extensions import db
from mechanics.forms.main import DescriptionForm, TagForm, CommentForm
from mechanics.models import User, Photo, Tag, Follow, Collect, Comment, Notification
from mechanics.notifications import push_comment_notification, push_collect_notification
from mechanics.utils import rename_image, resize_image, redirect_back, flash_errors
...
def search():
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['MECHANICS_SEARCH_RESULT_PER_PAGE']
    if category == 'user':
...
def show_notifications():
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['MECHANICS_NOTIFICATION_PER_PAGE']
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        filename_s = resize_image(f, filename, current_app.config['MECHANICS_PHOTO_SIZE']['small'])
        filename_m = resize_image(f, filename, current_app.config['MECHANICS_PHOTO_SIZE']['medium'])
        photo = Photo(
            filename=filename,
...
def show_photo(photo_id):
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['MECHANICS_COMMENT_PER_PAGE']
...
def show_collectors(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['MECHANICS_USER_PER_PAGE']
