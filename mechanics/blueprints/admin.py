...
from mechanics.forms.admin import EditProfileAdminForm
from mechanics.models import Role, User, Tag, Photo, Comment
...
def manage_user():
    ...
    per_page = current_app.config['MECHANICS_MANAGE_USER_PER_PAGE']
...
def manage_photo(order):
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['MECHANICS_MANAGE_PHOTO_PER_PAGE']
    order_rule = 'flag'
    if order == 'by_time':
...
def manage_tag():
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['MECHANICS_MANAGE_TAG_PER_PAGE']
...
def manage_comment(order):
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['MECHANICS_MANAGE_COMMENT_PER_PAGE']
    order_rule = 'flag'
    if order == 'by_time':
